...
def load_store() -> dict[str, Any]:
    ...
    if not DATA_FILE.exists():
        store: dict[str, Any] = {"profile": fresh_profile(), "feedback": [], "dailyContent": {}, "syllabus": None}
        save_store(store)
        return store
...
def ensure_daily_state(store: dict[str, Any]) -> None:
    ...
    profile["syllabus"] = store.get("syllabus", None)
...
def process_syllabus_text(text: str) -> dict[str, Any]:
    """
    Process syllabus text to extract structured data including dates, subjects, and topics.
    Uses pattern matching to organize the content intelligently.
    """
    ...
    subjects = {
        'physics': ['physics', 'phy', 'mechanics', 'thermodynamics', 'optics', 'electromagnetism'],
        'chemistry': ['chemistry', 'chem', 'organic', 'inorganic', 'physical chemistry'],
        'maths': ['maths', 'mathematics', 'math', 'algebra', 'calculus', 'geometry', 'trigonometry'],
        'biology': ['biology', 'bio', 'botany', 'zoology', 'genetics', 'ecology'],
        'english': ['english', 'grammar', 'literature', 'writing'],
        'computer': ['computer', 'cs', 'programming', 'algorithm', 'data structure'],
    }
    ...
    for line in lines:
        ...
        exam_keywords = ['jee', 'neet', 'board', 'exam', 'test', 'midterm', 'final', 'semester', 'quarterly']
        if any(keyword in line.lower() for keyword in exam_keywords):
            current_exam = line
            if line not in syllabus_data['exams']:
                syllabus_data['exams'].append(line)
            continue
        ...
        if found_subject:
            ...
            if found_subject not in syllabus_data['subjects']:
                syllabus_data['subjects'][found_subject] = {
                    'name': found_subject.title(),
                    'topics': []
                }
            continue
        
        # If we have a current subject and the line looks like a topic
        if current_subject and line and not any(c.isdigit() for c in line[:10]):
            topic_text = line.strip()
            if topic_text and len(topic_text) > 2:
                syllabus_data['subjects'][current_subject]['topics'].append(topic_text)
                syllabus_data['topics'].append({
                    'subject': current_subject,
                    'topic': topic_text,
                    'completed': False
                })
    
    # Group topics by date in schedule
    for item in syllabus_data['schedule']:
        for topic in syllabus_data['topics']:
            if item['description'].lower() in topic['topic'].lower() or topic['topic'].lower() in item['description'].lower():
...
class CookedRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_PUT(self) -> None:
        ...
        if self.path == "/api/profile/avatar":
            ...
            return
        if self.path == "/api/syllabus":
            self.handle_save_syllabus()
            return
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/api/support":
            ...
            return
        if self.path == "/api/syllabus":
            self.handle_fetch_syllabus()
            return
        if self.path == "/api/syllabus/process":
            self.handle_process_syllabus()
            return
        self.send_json(HTTPStatus.NOT_FOUND, {"error": "Endpoint not found."})
    ...
    def handle_save_syllabus(self) -> None:
        """PUT /api/syllabus  Save syllabus data."""
        payload = self.read_json()
        syllabus = payload.get("syllabus")
        if not syllabus:
            self.send_json(HTTPStatus.BAD_REQUEST, {"error": "Syllabus data is required."})
            return
        with STORE_LOCK:
            store = load_store()
            ensure_daily_state(store)
            store["profile"]["syllabus"] = syllabus
            store["syllabus"] = syllabus
            save_store(store)
            self.send_json(HTTPStatus.OK, store["profile"])

    def handle_fetch_syllabus(self) -> None:
        """GET /api/syllabus  Fetch saved syllabus data."""
        with STORE_LOCK:
            store = load_store()
            ensure_daily_state(store)
            save_store(store)
            syllabus = store["profile"].get("syllabus") or store.get("syllabus")
            self.send_json(HTTPStatus.OK, {"syllabus": syllabus})

    def handle_process_syllabus(self) -> None:
        ...
        if not text or not text.strip():
            ...
            return
        try:
            result = process_syllabus_text(text)
            self.send_json(HTTPStatus.OK, {"result": result})
